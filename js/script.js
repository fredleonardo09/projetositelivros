window.onload= function(){
    ...
    let htmlfull = '';
    fetch('http://localhost/LivrosBackEnd/conteudo/novos').then(response => response.json())
    .then(
        data => {             
            data.forEach( livro => {
                console.log(livro);                                                                
                ...
                if ( i == 7  ) {
			        ...
			        livrosanun.innerHTML = livrosanun.innerHTML + htmlfull;
                	i = 1;
			        htmlfull = '';
                }
                else { 
